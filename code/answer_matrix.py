...
BITSTRINGS_BASE = np.load(os.path.join(os.path.dirname(__file__), 'samples', 'data', 'bitstrings_base.npz'))['arr_0']
...
for circuit_idx in [1, 2, 5, 6, 8, 'ghz']:
    # 读取线路数据
    # 打开文件并读取内容
    with open(os.path.join(os.path.dirname(__file__), 'samples', 'circuit', f'circuit_{circuit_idx}.txt'), 'r') as file:
    ...
    # 读取测量结果
    bitstring_arr = np.load(os.path.join(os.path.dirname(__file__), 'samples', 'data', f'bitstrings_circuit_{circuit_idx}.npz'))['arr_0']
...
def correct(measure_prob_list, method="matrix_inversion", data_method="complex_state"):
    ...
    # 矩阵存储
    np.save(os.path.join(os.path.dirname(__file__), 'result','随机矩阵法结果',f'{method}_{data_method}.npy'), W_final)
